...
aliases:
  - /triage/
description: "Obsidian plugin for resurfacing unprocessed notes into a triage queue"
project:
  year: 2026
  category: /obsidian
  description: "content-rich note triage queue for Obsidian with keep, snooze, delete, and open actions"
  image: /visuals/project-thumbs/proj_triage_obsidian.png
---

[Triage](https://github.com/totocaster/triage-obsidian) is an [Obsidian](https://obsidian.md/) plugin for resurfacing unprocessed notes before they disappear into the vault.

Every vault accumulates loose material: fleeting notes, literature notes, clippings, and pages that never got a tag or a category. Triage gathers eligible notes into a scrolling queue of cards so I can decide what each one needs: keep it, snooze it, delete it, or open it and actually process the note.

## What it does

- Builds a queue from notes that match configurable include and exclude filters.
- Shows note cards with actual content previews, not just titles.
...
The plugin borrows some ergonomics from Anki: one focused card, numbered decisions, and intervals that feel quick enough to use. It is not a spaced-repetition tool, though. There is no memory model or ease factor. Snooze writes a plain due date to frontmatter, and the note returns when that date arrives.
